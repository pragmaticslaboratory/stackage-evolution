...
for indice,lts in enumerate(lts_list):
    print('--------------------------------------------------------------------------------------------')
    print('---------------------------------Start with the lts',lts,'----------------------------------')
    print('--------------------------------------------------------------------------------------------')
    file2 = io.open('C:/Users/nicol/Documents/GitHub/src/scrapy/packagebot/spiders/stackage.py', 'r', encoding="utf-8")
        ...
        if re.search('lts-',line):
            for jdx,ltswdot in enumerate(lts_list_wdot):      
                if re.search(ltswdot,line):     
                    stackage[idx] = stackage[idx].replace(ltswdot,lts_list_wdot[indice])

    file2.close()

    listToStr1 = ''.join([str(elem) for elem in stackage])
    file = io.open('C:/Users/nicol/Documents/GitHub/src/scrapy/packagebot/spiders/stackage.py', 'w', encoding="utf-8")
